...
class GLIF3Neuron(neuron.BaseNode):
    ...
    This implementation provides a GLIF3 neuron model
    that maintains full compatibility with both the NEST simulator's glif_psc model and the
    SpikingJelly framework. The GLIF3 model incorporates after-spike currents (ASC) for
    adaptation mechanisms and supports multiple synaptic time constants.
    
    Key Features:
    - NEST-compatible parameter handling and dynamics
    - Vectorized computations for improved performance
    - SpikingJelly framework integration for neural network construction
    - Exact integration methods for numerical stability
    - Support for multiple synaptic input types with different time constants
    ...
    def __init__(self, 
        ...
        **kwargs
        ...
        self.V_m_mV, self.V_th_mV, self.g_nS = V_m, V_th, g
        self.E_L_mV, self.C_m_pF, self.t_ref_ms = E_L, C_m, t_ref
        self.V_reset_mV, self.dt_ms = V_reset, dt
        
        # Compute derived membrane properties
        self.tau_m_ms = C_m / g  # Membrane time constant (ms)
        ...
        self._compute_integration_parameters(asc_decay, asc_amps, tau_syn, asc_init)
    ...
    def _compute_integration_parameters(self, asc_decay, asc_amps, tau_syn, asc_init):
        """
        Pre-compute all parameters required for numerically stable integration.
        
        This method calculates the discrete-time integration coefficients using
        exact analytical solutions of the underlying differential equations. This
        approach ensures numerical stability and maintains compatibility with
        the NEST simulator implementation.
        
        The integration uses matrix exponential methods for coupled differential
        equation systems, providing exact solutions within each time step.
        ...
        """
        h = self.dt_ms  # Integration time step
        ...
        # Compute refractory decay rates for ASC (NEST-compatible)
        # These determine how much ASC remains after the refractory period
        asc_refractory_decay_rates = np.exp(-asc_decay_array * self.t_ref_ms)
        ...
        P33 = np.exp(-h / self.tau_m_ms)  # Voltage decay factor
        P30 = (1.0 / self.C_m_pF) * (1.0 - P33) * self.tau_m_ms  # Current integration factor
        ...
        # Using NEST's IAFPropagatorAlpha logic for numerical stability
        P31, P32 = self._compute_synaptic_coupling_coefficients_nest_compatible(tau_syn_array, h)
        self.P31 = torch.from_numpy(P31)
        self.P32 = torch.from_numpy(P32)
        
        # === Additional Integration Parameters ===
        self.v_range = torch.tensor(self.V_th_mV - self.E_L_mV, dtype=torch.float32)
    ...
    def _compute_synaptic_coupling_coefficients_nest_compatible(self, tau_syn, h):
        """
        Calculate exact integration coefficients for synaptic current effects on membrane voltage.
        
        This method implements the NEST IAFPropagatorAlpha logic for exact integration
        of coupled differential equation systems.
        ...
        for i, tau_s in enumerate(tau_syn):
            # NEST's IAFPropagatorAlpha logic
            beta = tau_s * self.tau_m_ms / (self.tau_m_ms - tau_s)
            gamma = beta / self.C_m_pF
            
            # Regular case: tau_membrane ≠ tau_synaptic
            exp_h_tau_syn = np.exp(-h / tau_s)
            expm1_h_tau = np.expm1(h * (1/tau_s - 1/self.tau_m_ms))
    ...
    def _initialize_state_variables(self):
        ...
        Sets up the internal state of the neuron including membrane potential,
        after-spike currents, synaptic states, and refractory counter. All state
        variables are registered as memory attributes for proper handling by
        the SpikingJelly framework.
        """
        # Initialize membrane potential relative to leak reversal potential (NEST-compatible)
        # NEST stores U_ as relative to E_L_, so V_m - E_L
        initial_voltage = self.V_m_mV - self.E_L_mV
        self.U = torch.tensor(initial_voltage, dtype=torch.float32)
        
        # Initialize after-spike current state vector (NEST-compatible)
    ...
    def neuronal_charge(self, x: torch.Tensor, syn_inputs: Optional[torch.Tensor] = None):
        ...
        This method implements the core dynamics of the GLIF3 model using exact
        integration methods. It updates membrane voltage, after-spike currents,
        and synaptic states while respecting the refractory period constraints.
        
        The integration follows NEST simulator conventions for maximum compatibility
        ...
        not_refractory = self.ref_count <= 0.0
        
        # === Membrane Voltage Update (vectorized) ===
        # NEST-compatible ASC handling: calculate sum first, then update ASC values
        ...
        # Apply voltage update only when not refractory (vectorized)
        self.U = torch.where(not_refractory, voltage_update, self.U)
        
        # NEST-compatible ASC update: update ASC values after calculating contribution
        asc_update = self.asc * self.asc_decay_dt
        self.asc = torch.where(not_refractory, asc_update, self.asc)
    ...
    def neuronal_fire(self):
        """
        Determine whether the neuron generates a spike based on current state.
        
        Implements the threshold detection mechanism of the GLIF3 model. A spike
        is generated when the membrane potential exceeds the threshold and the
        neuron is not in its refractory period.
        
        Returns:
        --------
        torch.Tensor
            Boolean tensor indicating spike occurrence (True) or absence (False).
            Compatible with SpikingJelly's spike representation conventions.
        """
        # Calculate threshold relative to leak reversal potential (NEST-compatible)
        threshold_relative = self.V_th_mV - self.E_L_mV
        
        # Spike occurs when voltage exceeds threshold AND neuron is not refractory
        voltage_above_threshold = self.U >= threshold_relative
        not_in_refractory = self.ref_count <= 0.0
        
        return voltage_above_threshold & not_in_refractory
    
    def neuronal_reset(self, spike: torch.Tensor):
        """
        Reset neuronal state variables following spike generation.
        
        Implements the post-spike reset mechanisms of the GLIF3 model including
        voltage reset, after-spike current increments, and refractory period
        initiation. Uses conditional updates to maintain computational efficiency.
        
        Parameters:
        -----------
        spike : torch.Tensor
            Boolean tensor indicating which neurons have spiked and require reset.
        """
        # === Voltage Reset (NEST-compatible) ===
        # NEST stores V_reset_ as relative to E_L_, so we need to convert
        reset_voltage = self.V_reset_mV - self.E_L_mV
        self.U = torch.where(spike, torch.tensor(reset_voltage, dtype=torch.float32), self.U)
        
        # === After-Spike Current Update (NEST-compatible) ===
        # Reset ASC currents using NEST-compatible refractory decay mechanism
        # ASC = asc_amps + ASC * asc_refractory_decay_rates
        self.asc = torch.where(spike.unsqueeze(0), 
                              self.asc_amps + self.asc * self.asc_refractory_decay_rates, 
                              self.asc)
        
        # === Refractory Period Initiation ===
        # Set refractory counter to full refractory duration for spiking neurons
    ...
    def forward(self, x: torch.Tensor, syn_inputs: Optional[torch.Tensor] = None):
        ...
        This method orchestrates the complete neuron dynamics for one time step,
        including state updates, spike detection, and post-spike reset. It serves
        as the primary interface for the SpikingJelly framework.
        ...
        torch.Tensor
            ...
            Compatible with downstream SpikingJelly network components.
        """
        ...
        # Return spike as float for SpikingJelly compatibility
        return spike.float()
    ...
    def get_state(self):
        ...
        Provides comprehensive access to the internal state of the neuron including
        membrane potential, adaptation currents, synaptic states, and derived
        quantities. Useful for debugging, visualization, and scientific analysis.
    ...
    def extra_repr(self) -> str:
        """
        Generate concise string representation of neuron parameters for display.
        
        Used automatically by PyTorch when printing network architectures or
        individual neuron instances. Provides key parameter values for quick
        identification and debugging.
        
        Returns:
        --------
        str
            Formatted string containing essential neuron parameters.
        """
        return (f'V_th={self.V_th_mV:.1f}mV, tau_m={self.tau_m_ms:.1f}ms, '
                f'ASC_components={self.num_asc}, synaptic_types={self.num_syn}')
...
def load_parameters(json_file):
    ...
    Provides a convenient interface for loading pre-fitted GLIF model parameters
    from the Allen Brain Atlas Cell Types Database. These parameter files contain
    biophysically realistic values fitted to experimental electrophysiology data.
    ...
    Example:
    --------
    >>> params = load_parameters('cell_475622680_glif_psc.json')
    >>> neuron = GLIF3Neuron(**params)
    """
...
def create_glif3_from_params(params_file, dt=0.1):
    ...
    Streamlines the process of creating neurons with experimentally-derived parameters
    by automatically loading and applying the correct parameter values. This ensures
    consistency with published neuron characterizations.
    ...
    Example:
    --------
    >>> neuron = create_glif3_from_params('mouse_cell_params.json', dt=0.05)
    >>> response = neuron(torch.tensor(100.0))  # 100 pA current injection
    """
...
def test_fi_curve(neuron, current_range, duration=1000, dt=0.1, stim_start=200, stim_end=800):
    ...
    The F-I curve is a fundamental characterization of neuronal excitability, showing
    how firing frequency varies with injected current. This analysis is essential for
    validating model behavior against experimental observations and comparing different
    neuron types or parameter sets.
    
    The test applies current injection only during the specified time window within
    the total simulation duration, allowing observation of both transient responses
    and return to baseline. Firing frequency is calculated based only on spikes
    occurring during the stimulation period.
    ...
    current_range : iterable
        ...
        Typically ranges from subthreshold to suprathreshold values.
    ...
    Example:
    --------
    >>> neuron = create_glif3_from_params('cell_params.json')
    >>> currents = range(0, 500, 25)  # 0 to 500 pA in 25 pA steps
    >>> fi_data = test_fi_curve(neuron, currents, duration=1000, stim_start=200, stim_end=800)
    >>> for result in fi_data:
    ...     print(f"{result['current']} pA -> {result['frequency']:.1f} Hz")
    """
    results = []
...
def main():
    """
    Demonstrate and test the optimized GLIF3 neuron implementation.
    """
